            ...
            try:
                ...
                if len(self.stats_CPU) >= self.max_cached_rows:
                    ...
                    if int(self.database_thread_cursor.execute("select count(*) from memory").fetchone()[0]) >= self.max_rows_before_purge * 2:
                        self.database_thread_cursor.execute("delete from cpu where collection_date not in (select collection_date from cpu order by collection_date desc limit "+ str(self.max_rows_before_purge) +")")
                        self.database_thread_cursor.execute("delete from memory where collection_date not in (select collection_date from memory order by collection_date desc limit "+ str(self.max_rows_before_purge) +")")
                        self.database_thread_cursor.execute("delete from load where collection_date not in (select collection_date from load order by collection_date desc limit "+ str(self.max_rows_before_purge) +")")
                        self.database_thread_cursor.execute("delete from network where collection_date not in (select collection_date from network order by collection_date desc limit "+ str(self.max_rows_before_purge) +")")
